...
export class ChatController {
  ...
  @Get(':chatroomId')
  async findAllByChatroomId(@Param('chatroomId') chatroomId: string, @Query() pageOptionsDto?: PageOptionsDto) {
    const { results, total } = await this.chatService.find(chatroomId, pageOptionsDto);
    ...
    const { data, meta } = new PageDto(
      results.map(chat => new ChatResponseDto(chat, userMap.get(chat.userId))),
